...
async function appFetch(path,options={}){const headers=new Headers(options.headers||{});if(!['GET','HEAD'].includes((options.method||'GET').toUpperCase()))headers.set('X-CSRF-Token',document.querySelector('meta[name=csrf-token]').content);const response=await window.fetch(path,{...options,headers});if(response.status===401)window.location.assign('/login');return response;}
...
async function api(path,body){const response=await appFetch(path,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});let data;try{data=await response.json();}catch{throw new Error('requestError');}if(!response.ok)throw new Error(data.error||'requestError');return data;}
...
async function setLanguage(code){
  ...
  try{
    const response=await appFetch('/static/locales/'+encodeURIComponent(code)+'.json');if(!response.ok)throw new Error('translationError');const next=await response.json();
...
$('clearChat').onclick=async()=>{if(busy)return;stopSpeech();try{const response=await appFetch('/api/history',{method:'DELETE'});if(!response.ok)throw new Error('historyError');}catch(error){showError('chatError',error);return;}history=[];$('speechStatus').textContent='';draftFromResult=false;$('message').value='';renderMessages();updateSend();resizeComposer();$('chatError').textContent='';$('message').focus();};$('type').onclick=()=>{stopSpeech();$('message').focus();};
...
function renderLanguages(){
  ...
  for(const [,codes] of languageGroups){
...
async function refreshCapabilities(){
  try{const data=await(await appFetch('/api/speech/voices')).json();cloudVoices=data.voices||[];populateVoices();}catch{}
  try{languageAvailability=await(await appFetch('/api/languages')).json();renderLanguages();}catch{}
}
...
(async()=>{
  ...
  english=await(await appFetch('/static/locales/en.json')).json();catalog=english;renderLanguages();
  const saved=localStorage.getItem('farm-language')||'en';await setLanguage(saved in languages?saved:'en');
  try{const response=await appFetch('/api/history');if(!response.ok)throw new Error();const data=await response.json();history=data.messages;renderMessages();}catch{$('chatError').textContent=t('historyError');}
